...
class UsersController extends BackendSetupController
{
    ...
    public function request_forgot_password(Request $r)
    {
        ...
        if( $validator->fails() == false ){
            ...
            $email  = $r->input('email');
            $subject = 'Confirm '.$r->input('email').' on Backend Setup Lumen';
            $compose_email = "https://localhost/backend-setup/public/api/auth/forgot_password/".$user->secretcode."";
    ...
    public function request_email(Request $r)
    {
        ...
        if( $validator->fails() == false ){
            ...
            //1. mengirim Email ke usernya.
            $email  = $user->email;
            $subject = 'Confirm '.$user->email.' on Backend Setup Lumen';
            $compose_email = "https://192.168.1.4/backend-setup/public/api/auth/verify_email/".$user->secretcode."";
            $this->sendEmail($email, $subject, $compose_email);
    ...
    public function postRegister(Request $request)
    {
        ...
        $validator = Validator::make($input, [
            ...
            'username' => 'required|min:5|max:15|without_spaces|unique:users',
        ...
        $validator->after(function($validator) use ($input) {
            ...
            $role_count = UserRoles::where('name', $input['roles'])->count();
            if ($role_count == 0) {
    ...
    public function uploadImage(Request $request)
    {
        ...
        $validator = Validator::make($input,[   
            'image' => 'required|max:2064|mimes:jpg,jpeg,png',
        ]);
